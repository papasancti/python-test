  ...
  stages {
    stage('Download') {
      steps {
        sh 'rm -rf *'
        sh 'mkdir app'
        sh 'cd app && curl --no-check-certificate http://raw.githubusercontent.com/papasancti/python-test/main/webpage.py?token=GHSAT0AAAAAACOEUWNHC2OEZD6BEGEFLCR4ZOLMHCA'
      }
    }
